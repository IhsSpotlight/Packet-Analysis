...
class HijackDetector:
    ...
    def _check_ttl(self, session, key, rec):
        is_fwd = session.is_forward_packet(key)
        baseline = session.baseline_ttl if is_fwd else session.baseline_ttl_rev
        if baseline is None or rec.ttl is None:
            return None
        drift = abs(rec.ttl - baseline)
        ...
        return Alert(
            ...
            message=(f"TTL drift of {drift} on session {key} "
                     f"(baseline={baseline}, observed={rec.ttl}) — "
                     f"possible spoofed packet"),
            details={"baseline_ttl": baseline, "observed_ttl": rec.ttl,
    ...
    def _check_mac(self, session, key, rec):
        is_fwd = session.is_forward_packet(key)
        baseline = session.baseline_mac if is_fwd else session.baseline_mac_rev
        if baseline is None or rec.src_mac is None:
            return None
        if rec.src_mac == baseline:
            return None
        ...
        return Alert(
            ...
            message=(f"Source MAC changed mid-session on {key} "
                     f"(baseline={baseline}, observed={rec.src_mac}) — "
                     f"possible ARP spoofing / session takeover"),
            details={"baseline_mac": baseline, "observed_mac": rec.src_mac,
                      "direction": "forward" if is_fwd else "reverse",
                      "session_key": key},
        )
