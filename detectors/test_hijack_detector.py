...
def test_reverse_direction_mac_and_ttl_do_not_false_positive():
    """Regression test for the direction-blind baseline bug: outbound
    packets legitimately show YOUR NIC's MAC and TTL, while inbound
    packets legitimately show your GATEWAY's MAC and the remote host's
    (hop-reduced) TTL. That structural difference must never be flagged
    as a hijack on an ordinary bidirectional flow."""
    table = SessionTable()
    fired = []
    detector = HijackDetector(table, ttl_variance=5, on_alert=lambda a: fired.append(a))

    client_key = table.make_key("192.168.0.107", 65107, "104.18.39.21", 443, "TCP")
    server_key = table.make_key("104.18.39.21", 443, "192.168.0.107", 65107, "TCP")

    def feed_raw(key, flags, seq=1000, ttl=64, mac="aa:aa:aa:aa:aa:aa"):
        session = table.get_or_create(key)
        rec = PacketRecord(timestamp=time.time(), seq=seq, ack=0, ttl=ttl,
                            flags=flags, src_mac=mac, length=60)
        session.add_packet(rec, key)
        return detector.on_packet(session, key, rec)

    # forward: your own NIC's MAC, your OS's TTL
    feed_raw(client_key, "S", seq=1000, ttl=64, mac="b8:fb:b3:25:70:0e")
    # reverse: your gateway's MAC (last hop), remote host's hop-reduced TTL —
    # both DIFFERENT from the forward baseline, but this is normal
    feed_raw(server_key, "SA", seq=5000, ttl=52, mac="10:6f:d9:c2:fe:cb")
    feed_raw(client_key, "A", seq=1100, ttl=64, mac="b8:fb:b3:25:70:0e")
    feed_raw(server_key, "PA", seq=5100, ttl=52, mac="10:6f:d9:c2:fe:cb")

    mac_alerts = [a for a in fired if a.alert_type == "MAC_SWAP"]
    ttl_alerts = [a for a in fired if a.alert_type == "TTL_ANOMALY"]
    assert mac_alerts == [], f"expected no MAC_SWAP from normal fwd/rev MAC difference, got {mac_alerts}"
    assert ttl_alerts == [], f"expected no TTL_ANOMALY from normal fwd/rev TTL difference, got {ttl_alerts}"
    print("PASS: test_reverse_direction_mac_and_ttl_do_not_false_positive")


def test_real_mac_swap_within_reverse_direction_still_detected():
    """A genuine ARP-spoof-style hijack of the REVERSE direction (someone
    starts impersonating the gateway mid-session) must still be caught —
    the fix should only ignore the fwd-vs-rev difference, not disable
    same-direction consistency checking."""
    table = SessionTable()
    fired = []
    detector = HijackDetector(table, on_alert=lambda a: fired.append(a))
    ...
    def feed_raw(key, flags, seq=1000, mac="aa:aa:aa:aa:aa:aa"):
        session = table.get_or_create(key)
        rec = PacketRecord(timestamp=time.time(), seq=seq, ack=0, ttl=64,
                            flags=flags, src_mac=mac, length=60)
        session.add_packet(rec, key)
        return detector.on_packet(session, key, rec)

    feed_raw(client_key, "S", seq=1000, mac="b8:fb:b3:25:70:0e")
    feed_raw(server_key, "SA", seq=5000, mac="10:6f:d9:c2:fe:cb")     # establishes reverse baseline
    feed_raw(server_key, "PA", seq=5100, mac="66:66:66:66:66:66")     # reverse MAC changed — real hijack

    mac_alerts = [a for a in fired if a.alert_type == "MAC_SWAP"]
    assert len(mac_alerts) == 1, "a real same-direction MAC swap should still be caught"
    assert mac_alerts[0].details["direction"] == "reverse"
    print("PASS: test_real_mac_swap_within_reverse_direction_still_detected")
...
if __name__ == "__main__":
    ...
    test_reverse_direction_mac_and_ttl_do_not_false_positive()
    test_real_mac_swap_within_reverse_direction_still_detected()
