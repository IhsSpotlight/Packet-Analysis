...
def start_discovery_thread(scanner: DiscoveryScanner, cidr: str, interval_sec=300, on_hosts=None):
    """Runs scan_subnet + diff_and_alert on a timer, in a background thread.
    on_hosts, if given, is called with the raw discovered-host list every
    cycle — independent of diff_and_alert's alerting logic, so the SOC
    server can maintain a full asset inventory even for devices that are
    already registered (and therefore never generate an alert)."""
    def loop():
        while True:
            try:
